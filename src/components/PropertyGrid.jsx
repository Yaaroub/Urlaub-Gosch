  ...
  return (
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {properties.map((property) => {
        ...
        const imageUrl =
          getSafeImageUrl(property);

        const isKuschelwochen =
          ...
          >
            ...
            >
              <div className="relative overflow-hidden bg-slate-100">
                ...
                <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/[0.12] via-transparent to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />

                {showAvailabilityBadge && (
                  <span className="absolute left-3 top-3 z-10 rounded-full bg-emerald-600 px-2.5 py-1 text-xs font-semibold text-white shadow-sm">
                    Verfügbar
                  </span>
                )}
                ...
                {isKuschelwochen && (
                  <span
                    ...
                    aria-label="Dieses Objekt nimmt an den Ostsee-Kuschelwochen teil"
                  >
                    <Sparkles
                      aria-hidden="true"
...
function getSafeImageUrl(property) {
  ...
  /*
   * Alte Uploads wie /uploads/xyz.jpg
   * funktionieren online auf Vercel nicht,
   * wenn sie nicht wirklich in public/uploads
   * deployed wurden.
   *
   * Lokal lassen wir sie weiterhin zu.
   */
  if (
